...
Let `I*` be the set of finite input histories, including the empty history `[]`. Let `I+` be the nonempty histories.
...
where `beh_c(x)(i_1...i_n)` is the articulation emitted on the final transition after feeding that history from state `x`.

### 5.4 Finality theorem
...
A cohered result may become a coherer in a later event through path composition:

```text
s --φ→ o --ψ→ u
```

Role succession across events does not imply role permutation within one event.
