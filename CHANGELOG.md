...
|---------|-----|---|---|---|-------|------|
| 0.12.0 | A- | A- | A | A- | L7 | factorized-β measurement route + CDD cnos-3.82.0 platform refresh. The pre-registered freedom-seam experiment (wave #73: engine #74, harness #75) ran on the credentialed CI witness (k=3, five held-out targets) and returned a **terminal FAIL** — discrimination held (B1/B2/B3) but consistency did not (A1/A2/A3); A3 shows independent witnesses disagree on the local semantic relation itself, localizing the residual variance to genuine semantic judgment over under-specified citation relations. Third and strongest falsification of consistency-improvement: even fixed-locus local adjudication did not cross the gate. The meter is retained as measurement infrastructure with **no consistency claim**; the line is stopped (no v3.2.5 without a fresh operator dispatch) and pivots to defect/ambiguity harvesting (#76). Platform: `cnos.cdd`/`cds`/`handoff` skills vendored + integrity-checked at cnos 3.82.0 (was reference-only; pin 982860df→fd1d654), `cdd-artifact-validate` + `validate-release-gate.sh` wired, legacy cycles 50–54 migrated, pin-format guard added (#71). §5.2 single-session δ-as-γ throughout → γ floored at A− per `release/SKILL.md` §3.8. The α/γ scalar witness path and `runtime/SELF-MEASURE.md` are untouched (factorized-β is strictly additive). γ PRA (`docs/gamma/cdd/0.12.0/POST-RELEASE-ASSESSMENT.md`) ratifies these grades (α A−, β A, γ A−, C_Σ A−, L7) as **final**. |
