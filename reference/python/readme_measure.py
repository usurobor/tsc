...
class MeasurementResult:
    ...
    failed_witness: str | None
    alpha: float | None
    beta: float | None
    gamma: float | None
    c_sigma: float | None
    alpha_details: dict[str, Any] | None
    beta_details: dict[str, Any] | None
    ...
    return WitnessResult(
        ...
        details={
            "total_internal_links": len(
                [l for l in links if not l[1].startswith("http") and not l[1].startswith("#")]
            ),
...
def check_staleness_witness(readme_text: str, cli_path: Path | None = None) -> WitnessResult:
    ...
    return WitnessResult(name="staleness", passed=len(issues) == 0, details={"issues": issues})
...
def check_completeness_witness(readme_text: str) -> WitnessResult:
    ...
    return WitnessResult(
        ...
        details={"required": required_sections, "missing": missing, "found_headers": h2_headers},
    )
...
def run_witnesses(
    readme_text: str, base_path: Path, cli_path: Path | None = None
) -> tuple[list[WitnessResult], str, str | None]:
    """
...
def measure_gamma(
    alpha: float, beta: float, prev_measurement: MeasurementResult | None
    ...
    if prev_measurement and prev_measurement.c_sigma is not None:
        ...
        details = {"baseline_health": baseline_health, "formula": "sqrt(alpha * beta)"}
    ...
    try:
        ...
        for line in result.stdout.strip().split("\n"):
            if "|" in line:
                ...
                commits.append({"hash": hash_val, "date": date, "subject": subject})
...
def get_readme_at_commit(repo_path: Path, commit_hash: str) -> str | None:
...
def measure_readme(
    ...
    prev_measurement: MeasurementResult | None = None,
) -> MeasurementResult:
    """
...
def save_measurement(result: MeasurementResult, output_path: Path):
    ...
    data = {
        ...
        "witnesses": [
            {"name": w.name, "passed": w.passed, "details": w.details} for w in result.witnesses
        ],
        "measurements": {
            "alpha": result.alpha,
            "beta": result.beta,
        ...
        "details": {"alpha": result.alpha_details, "beta": result.beta_details},
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Measure README coherence using TSC methodology")
    parser.add_argument("readme", type=Path, help="Path to README.md")
    parser.add_argument("--repo", type=Path, help="Path to git repo root (for history)")
    parser.add_argument("--cli", type=Path, help="Path to CLI file (for staleness check)")
    parser.add_argument("--output", type=Path, help="Output JSON file")

    args = parser.parse_args()

    # Measure
    result = measure_readme(readme_path=args.readme, repo_path=args.repo, cli_path=args.cli)
