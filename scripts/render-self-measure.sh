...
on:
  ...
  # for recovery, backfill, and environments that cannot push tags.
  workflow_dispatch:
    inputs:
      version:
...
{ledger_build_steps}
      ...
      - name: Append ledger row and push
        run: |
          ...
          version="${{version#v}}"
          # Dispatch path: materialize the tag if it does not exist yet.
          # (A token-pushed tag deliberately does not cascade into other
